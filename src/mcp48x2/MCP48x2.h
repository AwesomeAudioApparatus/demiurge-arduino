...
class MCP48x2 {
  public:
     ...
     */
    MCP48x2(unsigned char mcpModel, spi_device_handle_t spi);
    MCP48x2(unsigned char mcpModel, spi_device_handle_t spi, unsigned int ldacPin);
    // config = will use the four high bits of the config byte as config bits for the word sent to MCP 
    void send(unsigned char config, unsigned int data);
  ...
  private:
    ...
    void _init(unsigned char mcpModel, spi_device_handle_t spi, unsigned int ldacPin);

    unsigned char _mcpModel;
